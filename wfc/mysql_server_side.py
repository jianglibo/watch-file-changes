...
import common_util
from .global_static import PyGlobal, EMPTY_PASSWORD
...
def main(action_in, args_ary):
    if action == 'MysqlExtraFile':
        ...
        common_util.send_lines_to_client(get_mysql_variables(args_ary))
    else:
        common_util.common_action_handler(action_in, args_ary)
...
def invoke_mysql_sql_command(sql, plain_password, combine_error=False):  # pylint: disable=W0613
    cmd_dict = get_sql_commandline(sql, plain_password)
    return common_util.subprocess_checkout_print_error(cmd_dict['cmd_line'])


def get_mysql_variables(variable_names=None, plain_password=None):  # pylint: disable=W0613
    result = invoke_mysql_sql_command(
